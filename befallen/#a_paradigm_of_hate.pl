...
sub EVENT_SAY {
	...
	if ($text =~ /hail/i) {
		quest::whisper("You must turn in either the left, right, or both hands. Turning in just one hand, left or right, will unlock Partial Anger (a chance at an illusion item). Turning in both hands will unlock the main boss. Once unlocked, simply hail me to respawn the fights after their cooldowns.");

		if (quest::get_data($both_hands)) {
			my $left_link = quest::silent_saylink("Left");
			my $right_link = quest::silent_saylink("Right");
			my $both_link = quest::silent_saylink("Both");
			quest::whisper("What fight would you like to do?");
			quest::whisper("$left_link hand fight (Partial Anger)");
			quest::whisper("$right_link hand fight (Partial Anger)");
			quest::whisper("$both_link hands fight (The Main Event)");
		} elsif (quest::get_data($left_hand)) {
			my $left_link = quest::silent_saylink("Left");
			quest::whisper("Would you like to respawn the left hand turn-in?");
			quest::whisper("$left_link hand fight (Partial Anger)");
		} elsif (quest::get_data($right_hand)) {
			my $right_link = quest::silent_saylink("Right");
			quest::whisper("Would you like to respawn the right hand turn-in?");
			quest::whisper("$right_link hand fight (Partial Anger)");
		}
	}

	if ($text =~ /right/i) {
		if (quest::get_data($right_hand) == 1 || quest::get_data($both_hands) == 1) {
			if (!$right_hand_lock) {
				# Lockout in seconds (1 hour)
				$client->SetBucket("-right_lockout", 1, 30);
		...
		if (quest::get_data($left_hand) == 1 || quest::get_data($both_hands) == 1) {
			if (!$left_hand_lock) {
				# Lockout in seconds (1 hour)
				$client->SetBucket("-left_lockout", 1, 30);
				quest::spawn2($partial_anger_id, 0, 0, $partial_anger_x, $partial_anger_y, $partial_anger_z, $partial_anger_h);
			} else {
		...
		if ($both_hand_lock) {
			...
			if (quest::get_data($both_hands) == 1) {
				# Lockout in seconds (1 hour)
				$client->SetBucket("-heat_lockout", 1, 30);
