# ==========================================
# a_cranberry_clucker - Featherblight Add
# NPCID: 2264
...
sub EVENT_TIMER {
    if ($timer eq "detect_era") {
        ...
        quest::debug("[Cranberry Clucker] Detected era: $era");

        eval { plugin::ApplyEraStats($npc, $era, 0); };  # Trash mob
        if ($@) {
            quest::debug("[Cranberry Clucker] ApplyEraStats failed: $@");
        } else {
            quest::debug("[Cranberry Clucker] ApplyEraStats applied (trash tier).");
        }
        ...
        if ($era eq "antonica") {
            ...
            quest::debug("[Cranberry Clucker] Antonica debuff: -50% HP/damage.");
        }
        ...
        if ($era eq "pop") {
            ...
            #quest::debug("[Cranberry Clucker] PoP buff: +125m HP and +25% damage.");
        }
    }
}