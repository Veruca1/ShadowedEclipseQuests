...
# === Crypt of Decay - Trash Kill -> Essence Tracking ===
...
# - Keys are zone-specific (CoDecay_*).
...
sub EVENT_DEATH_COMPLETE {
    ...
    foreach my $pc (@ip_clients) {
        ...
        my $essence_key    = "CoDecay_EssenceCount_${cid}";
        my $completion_key = "CoDecay_EssenceComplete_${cid}";

        my $count = quest::get_data($essence_key) || 0;
        $count++;
        quest::set_data($essence_key, $count);

        $pc->Message(15, "You have gathered an essence from CoDecay [$count / 1000]");

        if ($count >= 1000) {
            quest::set_data($completion_key, 1);
            $pc->Message(13, "You have gathered 1000 essences from CoDecay! A reward chest appears!");
