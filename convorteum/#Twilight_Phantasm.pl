# ===========================================================
# 2224.pl – Tower of Shattered Lanterns
# Floor 3 Mini Boss (Twilight Phantasm)
# -----------------------------------------------------------
# • Invulnerable until a nearby player has Obsidian Key (57161)
# • 50% chance to drop era loot on first combat engage
# • Grants Mini Essence flag + triggers HandleEssence
# • Full combat/spell suppression when locked
# ===========================================================

my $TOWER_KEY_ID   = 57161;   # Obsidian Key (Floor 2 → 3)
my $CHECK_RADIUS   = 250;
my $CHECK_INTERVAL = 5;
my $LOOT_SEEDED    = 0;       # prevent double seeding
...
sub EVENT_SPAWN {
    return unless $npc;
    _lock_if_unlocked();
    quest::settimer("f3_keycheck",$CHECK_INTERVAL);
}
...
sub EVENT_DEATH_COMPLETE {
    ...
    my $flag = "CONV_F3_Mini2224_${cid}";

    unless (quest::get_data($flag)) {
        quest::set_data($flag,1);
        $client->Message(13,"✨ You have absorbed the essence of the Twilight Phantasm!");
    }
