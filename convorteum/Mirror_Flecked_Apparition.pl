# ===========================================================
# Tower of Shattered Lanterns - Floor 3 Trash Logic
# NPCs: 2220–2223
# -----------------------------------------------------------
# • Invulnerable until player with Obsidian Key (57161) is nearby
# • Unlocks for valid keyholders
# • Grants Floor 3 essence progress (max 12)
# • Calls plugin::HandleEssence to manage Phantasmal Key (57162)
# ===========================================================
...
sub EVENT_DEATH_COMPLETE {
    ...
    my $ess   = "CONV_F3_EssenceCount_${cid}";
    my $count = quest::get_data($ess) || 0;

    if ($count < 12) {
        $count++;
        quest::set_data($ess, $count);
        $client->Message(15, "You have gathered an essence of Floor 3 [$count/12]");
    }

    # ✅ Check for Phantasmal Key unlock conditions
