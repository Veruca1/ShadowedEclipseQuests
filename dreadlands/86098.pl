sub EVENT_SAY {
    if ($text =~ /hail/i) {
        ...
        # Step 1: Get loottable_id for the given NPC name
        ...
        if ($sth_loottable->execute($name)) {
            if (my @loottable_row = $sth_loottable->fetchrow_array()) {
                ...
                # Step 2: Get lootdrop_ids from loottable_entries
                ...
                if ($sth_lootdrop->execute($loottable_id)) {
                    ...
                    while (my @lootdrop_row = $sth_lootdrop->fetchrow_array()) {
                        ...
                        # Step 3: Get item_ids from lootdrop_entries
                ...
                quest::whisper("No loottable found for NPC name '$name'.");
            }
        } else {
            quest::whisper("Database error: Unable to retrieve loottable information. Query: $loottable_query");
        }
