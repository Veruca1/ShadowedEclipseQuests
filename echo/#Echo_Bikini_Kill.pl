...
sub EVENT_COMBAT {
    if ($combat_state == 1) {  # When combat starts
        ...
        $npc->ModifyNPCStat("ac", 30000);  # Increase AC to make #Echo_Bikini_Kill tougher to hit
        $npc->CastSpell(40727, $npc->GetID(), 10, 0);  # Cast Hundred Fists Discipline at the start of combat
...
sub EVENT_DEATH_COMPLETE {
    quest::shout("I cannot believe this, no matter, You will submit to Nyseria!");
    ...
    if ($npc->IsEngaged()) {  # Check if the NPC is in combat
        ...
        if ($timer eq "special_ability") {
            ...
            if ($random == 0) {
                ...
                $npc->CastSpell(40727, $npc->GetID(), 10, 0);  # Cast Hundred Fists Discipline
            } elsif ($random == 1) {
                ...
                $npc->ModifyNPCStat("max_hp", 20000000);  # Temporary increase in hit points
...
sub EVENT_HP {
    if ($npc->IsEngaged() && $npc->GetHPRatio() <= 20) {  # When HP drops to 20% and in combat
        ...
        $npc->CastSpell(13250, $npc->GetID(), 10, 0);  # Cast Lightning Shock spell ID 466
    }
}