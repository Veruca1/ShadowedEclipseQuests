    ...
    if ($timer eq "charm_timer") {
        ...
        if ($combat_state == 1) {
            ...
            if ($random_player) {
                ...
                $npc->CastSpell(40656, $random_player->GetID(), 10, 0); 
...
sub EVENT_COMBAT {
    ...
    if ($combat_state == 1) {
        ...
        $npc->ModifyNPCStat("ac", 25000);  # Increases AC temporarily
        $npc->ModifyNPCStat("max_hp", 4000000);  # Increases HP temporarily
        ...
        $npc->ModifyNPCStat("ac", 15000);  # Resets AC back to original value
        $npc->ModifyNPCStat("max_hp", 1700000);  # Resets HP back to original value
...
sub cast_syvelians_anti_magic_aria {
    ...
    foreach my $player (@player_list) {
        if (!$player->IsBot()) {
            ...
            $npc->SpellFinished(955, $player, 1);  # 1 indicates unresisted
