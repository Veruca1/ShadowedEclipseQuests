...
sub EVENT_TIMER {
    if ($timer eq "summon_pet") {
        ...
        my $z = $npc->GetZ();
        my $h = $npc->GetHeading();
        quest::spawn2(1934, 0, 0, $x, $y, $z, $h); # Summons minion with NPC ID 63026
        quest::stoptimer("summon_pet");
    }
    elsif ($timer eq "spectral_shroud") {
        quest::modifynpcstat("AC", 25000); # Resets AC to normal
        quest::modifynpcstat("regen", 900); # Resets regen to normal
...
sub summon_minions {
    ...
    my $z = $npc->GetZ();
    my $h = $npc->GetHeading();
    quest::spawn2(1934, 0, 0, $x, $y, $z, $h); # Summons a minion with NPC ID 63026
}

sub EVENT_SIGNAL {
    if ($signal == 1) {
        quest::modifynpcstat("AC", 30000); # Increases AC temporarily
        quest::modifynpcstat("regen", 2000); # Increases HP regen temporarily
