...
sub EVENT_SAY {
    ...
    if ($text =~ /hail/i) {
        if (quest::get_data($flag)) {
            quest::whisper("Dobby sees you have freed him! Are you ready to play Dobby Says? Just say 'ready' to begin!");
        } else {
...
sub EVENT_ITEM {
    ...
    if (plugin::check_handin(\%itemcount, 675 => 1)) {  # Sock Item ID: 675
        ...
        quest::whisper("Master has given Dobby a sock! Dobby is FREE! Oh, happiest of days! But wait—Master must play Dobby Says! Yes, yes! Hail Dobby and say 'ready' when Master is prepared to begin!");
    }
    # Check if the player has already handed in item 700
        ...
        quest::whisper("Oh no, no, no! Dobby does not need this! Take it back, please, before Dobby gets in trouble!");
...
sub EVENT_SIGNAL {
    ...
    if ($signal == 4) { start_inferi_event(); }  # 1778 killed, start Inferi event

    if ($signal == 2) {  # Inferi killed
    ...
    if ($signal == 5) { start_fiendfyre_event(); }  # Start Fiendfyre event

    if ($signal == 6) {  # Fiendfyre killed
    ...
    if ($signal == 7) { start_corrupted_event(); }  # Start Corrupted event

    if ($signal == 8) {  # Corrupted killed
    ...
    if ($signal == 3) {  # Mrs. Norris Spawn Chance
    ...
    if ($signal == 9) {  # Spawn Bellatrix Lestrange
