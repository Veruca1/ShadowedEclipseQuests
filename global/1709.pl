sub EVENT_SPAWN {
    ...
    quest::whisper("Casting Aura of Insight on self and group!");

    # Cast Aura of Insight (spell ID 8926) on the bard itself
    $npc->CastSpell(8926, $npc->GetID());
    quest::whisper("Casting Aura of Insight (spell 8926) on NPC: " . $npc->GetName());  # Debugging log for NPC
    ...
    if ($group) {
        ...
        for (my $i = 0; $i < $group->GroupCount(); $i++) {
            ...
            if ($member && $member->IsClient()) {
                ...
                quest::whisper("Casting Aura of Insight (spell 8939) on player: " . $member->GetName());  # Debugging log for player
            }
            elsif ($member && $member->IsBot()) {
                # Cast the aura on each bot as well
                quest::castspell(8939, $member->GetID());
                quest::whisper("Casting Aura of Insight (spell 8939) on bot: " . $member->GetName());  # Debugging log for bot
            }
        }
        quest::whisper("Casting Aura of Insight on the entire group!");  # Confirmation of group cast
    } else {
        quest::whisper("No group found for this NPC!");  # Debugging: No group found
    }
}
