...
my %zones = (
    "thedeep" => "The Deep Upside Down",  # Updated zone long name
);

my %zone_versions = (
    "thedeep" => {
        1 => "The Deep Upside Down",  # Updated zone version
    },
);
...
sub EVENT_SAY {
    ...
    if ($text =~ /hail/i) {
        ...
        plugin::Whisper("The Deep Versions:");
        my @deep_versions = ("The Deep Upside Down");
        foreach my $version_name (@deep_versions) {
            plugin::Whisper("  - " . quest::saylink($version_name, 1, $version_name));
        }
        plugin::Whisper("Say the name to create a DZ.");
    }

    elsif ($text =~ /^(The Deep Upside Down)$/i) {
        my $zone_name = $1;
        foreach my $version (keys %{$zone_versions{"thedeep"}}) {
            if ($zone_versions{"thedeep"}->{$version} eq $zone_name) {
                my $expedition_name = $expedition_name_prefix . "thedeep";
                my $dz = $client->CreateExpedition("thedeep", $version, $dz_duration, $expedition_name, $min_players, $max_players);
                if ($dz) {
