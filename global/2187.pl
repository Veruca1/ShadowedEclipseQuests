...
my %zones = (
    "ssratemple" => "Ssraeshza Temple Upside Down",
);

my %zone_versions = (
    "ssratemple" => {
        1 => "Ssraeshza Temple Upside Down",
    },
);
...
sub EVENT_SAY {
    ...
    if ($text =~ /hail/i) {
        ...
        plugin::Whisper("Ssraeshza Temple Versions:");
        my @ssra_versions = ("Ssraeshza Temple Upside Down");
        foreach my $version_name (@ssra_versions) {
            plugin::Whisper("  - " . quest::saylink($version_name, 1, $version_name));
        }
        plugin::Whisper("Say the name to create a DZ.");
    }

    elsif ($text =~ /^(Ssraeshza Temple Upside Down)$/i) {
        my $zone_name = $1;
        foreach my $version (keys %{$zone_versions{"ssratemple"}}) {
            if ($zone_versions{"ssratemple"}->{$version} eq $zone_name) {
                my $expedition_name = $expedition_name_prefix . "ssratemple";
                my $dz = $client->CreateExpedition("ssratemple", $version, $dz_duration, $expedition_name, $min_players, $max_players);
                if ($dz) {
