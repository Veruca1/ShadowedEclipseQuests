sub EVENT_ITEM {
    ...
    if (plugin::check_handin(\%itemcount, 669 => 1, 670 => 1)) {
        ...
        plugin::Whisper("Ladies and gentlemen, warriors of legend, we have a **CONTENDER!** You have earned your place in the arena! When you're ready to face your fate, hail me and click " . quest::saylink("READY TO RUMBLE") . "!");
    } 
    # Rematch Handin Logic
    elsif (plugin::check_handin(\%itemcount, 672 => 1)) {
        quest::set_data("$char_id-rematch-1757", 1);
        plugin::Whisper("You've earned a rematch with the mighty Statue of Rallos Zek! Hail me when you're ready.");
    }
    elsif (plugin::check_handin(\%itemcount, 673 => 1)) {
        quest::set_data("$char_id-rematch-1756", 1);
        plugin::Whisper("You've earned a rematch with Derakor the Vindicator! Hail me when you're ready.");
    }
    elsif (plugin::check_handin(\%itemcount, 674 => 1)) {
    quest::set_data("$char_id-rematch-1758", 1);
    plugin::Whisper("You've earned a rematch with Overseer Hragveld Frostbane! Hail me when you're ready.");
...
sub EVENT_SAY {
    ...
    if ($text =~ /hail/i) {
        if (quest::get_data($flag)) {
            plugin::Whisper("**ARE YOU READY?!** Are you **really** ready? If so, click " . quest::saylink("READY TO RUMBLE") . " and prepare for the fight of your life!");
        } else {
            plugin::Whisper("Sorry, kid! This is the **big leagues**, and stepping into the ring doesn't come cheap! It takes **proof of your skill** AND a **hefty price of admission** to earn your shot at glory!");
        }
    }

    if ($text =~ /ready to rumble/i) {
        if (quest::get_data($flag)) {
            ...
            if (!$last_attempt_time || ($current_time - $last_attempt_time) > $cooldown_time) {
                ...
                plugin::Whisper("**LET'S GET READY TO RUMBLEEEEEE!!!**");
                ...
                plugin::Whisper("You're eager, I love it! But you've gotta wait just a little longer! **Time remaining:** " . int($remaining_time / 60) . " minutes, " . ($remaining_time % 60) . " seconds.");
            }
        } else {
            plugin::Whisper("You may have the heart of a warrior, but you still need to **prove yourself** before stepping into the arena!");
        }
    }
    ...
    if ($text =~ /hail/i) {
        plugin::Whisper("If you seek a rematch, select your opponent:");
        foreach my $name (keys %rematches) {
            my $npc_id = $rematches{$name}->{npc_id};
            if (quest::get_data("$char_id-rematch-$npc_id")) {
                plugin::Whisper(quest::saylink("Rematch with $name", 1));
            }
        }
    }

    foreach my $name (keys %rematches) {
        if ($text =~ /Rematch with $name/i) {
            my $npc_id = $rematches{$name}->{npc_id};
            my $cooldown_key = "$char_id-rematch-cooldown-$npc_id";
            my $last_attempt_time = quest::get_data($cooldown_key);
            my $current_time = time();

            if (!$last_attempt_time || ($current_time - $last_attempt_time) > 600) {
                quest::set_data($cooldown_key, $current_time);
                my ($x, $y, $z, $h) = @{$rematches{$name}->{loc}};
                quest::spawn2($npc_id, 0, 0, $x, $y, $z, $h);
                plugin::Whisper("$name has been summoned! Prepare yourself!");
            }
            else {
                my $remaining_time = 600 - ($current_time - $last_attempt_time);
                plugin::Whisper("You must wait another " . int($remaining_time / 60) . " minutes and " . ($remaining_time % 60) . " seconds before your next rematch.");
            }
        }
    }
}
...
sub EVENT_SIGNAL {
    ...
    if (!$char_id) {
        plugin::Whisper("Error: No valid player found for tracking signal 7.");
        return;
    }
    ...
    if ($signal == 1) {
        ...
        if ($signal_count == 4) { quest::shout("The crowd is getting restless! Who will fall first?"); }
        if ($signal_count == 7) { quest::shout("It’s turning into a real slugfest out there!"); }
        if ($signal_count == 10) { quest::shout("Ohh! That had to hurt! They’re giving it their all!"); }
        if ($signal_count == 15) { quest::shout("This fight is far from over! Who will emerge victorious?"); }
    ...
    if ($signal == 3) {
        ...
        if ($signal_3_count == 1) { quest::shout("What a battle! Neither side is backing down!"); }
        if ($signal_3_count == 4) { quest::shout("The hits just keep coming! Can they keep this pace up?"); }
        if ($signal_3_count == 12) { quest::shout("What a display of endurance! They're refusing to go down!"); }
        if ($signal_3_count == 15) { quest::shout("Unbelievable! This battle is reaching legendary status!"); }

        if ($signal_3_count == 16) {
