...
sub EVENT_TIMER {
    if ($timer eq "summon_pet") {
        ...
        if (!$pet_id) {
            my $pet = quest::spawn2(44060, 0, 0, 18.99, -24.08, 2.50, 0.00);  # Spawn the pet NPC (ID 44060) at the specified location
...
sub EVENT_COMBAT {
    if ($combat_state == 1) {
        if (my $pet_entity = $entity_list->GetNPCByNPCTypeID(44060)) {  # Get the pet entity by its NPC Type ID
            if (my $target = $npc->GetHateTop()) {
                $pet_entity->AddToHateList($target, 1);  # Sync aggro with the master
            }
        }
