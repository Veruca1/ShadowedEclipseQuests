...
sub EVENT_TIMER {
    if ($timer eq "summon_pet") {
        ...
        if (!$pet_id) {
            my $pet = quest::spawn2(44140, 0, 0, 19.27, 21.54, 4.06, 258.75);  # Spawn the pet NPC (ID 44140) at the specified location
...
sub EVENT_COMBAT {
    if ($combat_state == 1) {
        if (my $pet_entity = $entity_list->GetNPCByNPCTypeID(44140)) {  # Get the pet entity by its NPC Type ID
            if (my $target = $npc->GetHateTop()) {
                $pet_entity->AddToHateList($target, 1);  # Sync aggro with the master
            }
        }
