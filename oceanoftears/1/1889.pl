sub EVENT_SAY {
    ...
    my $flag = "$char_id-seb_bunny_event";  # Updated flag name
    my $cooldown_flag = "$char_id-seb_bunny_cooldown";  # Updated cooldown flag
    ...
    if ($text =~ /ready/i && quest::get_data($flag)) {
        ...
        if (!$last_summon_time || ($current_time - $last_summon_time) > $cooldown_time) {
            ...
            quest::spawn2(1875, 0, 0, -1211.03, -2064.74, -210.94, 203);
        } else {
...
sub EVENT_ITEM {
    ...
    if (plugin::check_handin(\%itemcount, 941 => 1)) {  # Keeping 941 as the basket ID
