sub EVENT_ITEM {
    ...
    my $flag = "$char_id-remainsofaking_flag";  # Unique flag identifier for this NPC

    # Check if the player handed in the required item (Item ID 32479)
    if (plugin::check_handin(\%itemcount, 32479 => 1)) {
        # Set the flag indicating the player has completed the hand-in
        quest::set_data($flag, 1);

        plugin::Whisper("The ancient king stirs... You may now hail me to proceed.");
    } else {
...
sub EVENT_SAY {
    my $char_id = $client->CharacterID();  # Get the character's unique ID
    my $flag = "$char_id-remainsofaking_flag";  # The flag for this NPC
    my $cooldown_key = "$char_id-remainsofaking_hail_cd";  # A unique cooldown key
    ...
    if ($text =~ /hail/i) {
        if (quest::get_data($flag)) {
            ...
            if (!$last_hail_time || ($current_time - $last_hail_time) > $cooldown_time) {
                ...
                plugin::Whisper("The remains shudder as Rile Sathir materializes...");
                # Spawn NPC 1361 (Rile Sathir) at the remains' current location
                my $x = $npc->GetX();
                my $y = $npc->GetY();
                my $z = $npc->GetZ();
                my $h = $npc->GetHeading();
                quest::spawn2(1361, 0, 0, $x, $y, $z, $h);
            } else {
