...
-- - Use only for standard trash, not event controllers or bosses.
-- ===============================================================
...
function event_spawn(e)
    ...
    local prisoner = eq.get_entity_list():GetMobByNpcTypeID(201463)
    if prisoner.valid then
        if prisoner:CalculateDistance(npc:GetX(), npc:GetY(), npc:GetZ()) < 200 then
            npc:AddToHateList(prisoner, 1)
        end
    end
end