...
function event_say(e)
	...
	if (qglobals["pop_poj_mavuin"] ~= nil) then
		if (e.message:findi("hail")) then
			e.self:Emote(" fixes you with a dark, piercing gaze. 'What do you want, mortal? Are you [" .. eq.say_link("prepared", false, "prepared") .. "]?'");

		elseif (e.message:findi("prepared")) then
			e.self:Say("Very well. When you are ready, you may [" ..
				eq.say_link("ready to begin the trial of torture", false, "begin the trial of torture") ..
				"]. Only when a wraith of agony dies will the prisoners feel any relief. Take care to find and kill it quickly, lest their torment overcome them. We shall judge the mark of your success.");
	...
	if (torture_flag == 0) then
		...
		local inst_id = eq.get_zone_instance_id(); -- ✅ Correct way to get instance ID

		-- Move the Player and/or their Group to the trial room
		if (trial_group ~= nil and trial_group.valid) then
			...
			if (inst_id ~= 0) then
				e.other:MovePCInstance(201, inst_id, 729, -1119, 88, 128); -- Zone: pojustice
			else
				e.other:MovePC(201, 729, -1119, 88, 128); -- fallback if not in DZ
			end
		end
		...
		e.self:Say("I'm sorry, the Trial of Torture is currently unavailable to you.");
	end
		...
		elseif (e.message:findi("i seek knowledge")) then
			...
			if (has_six == 1 and not e.other:HasItem(31599)) then
	...
	if (e.timer == "ejecttimer") then
		...
		if (trial_group ~= nil and trial_group.valid) then
            ...
            if (client_e ~= nil and client_e.valid) then
				local inst_id = client_e:GetInstanceID();
                client_e:MovePCInstance(201, inst_id, 456, 825, 9, 360); -- Zone: pojustice
			...
			if (mob_v ~= nil and mob_v.valid and mob_v:IsClient()) then
				...
				if (client_v.valid) then
					...
					if (client_v:CalculateDistance(src_x, src_y, src_z) <= distance) then
						...
						client_v:MovePCInstance(201, inst_id, tgt_x, tgt_y, tgt_z, tgt_h); -- Zone: pojustice
