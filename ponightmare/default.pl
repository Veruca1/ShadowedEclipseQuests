# ===========================================================
# default.pl — Plane of Nightmare (PoN)
...
sub EVENT_SPAWN {
    ...
    $exclusion_list->{$_} = 1 for (
        204036,  # Etumer
        204052,  # Wailing Sister
        204039,  # Mujaki the Devourer
        204037,  # #Thelin_Poxbourne
        204067,  # __Thelin_Poxbourne
        204016,  # Thelin_Poxbourne
        204058   # Hedge_Trigger
    );
...
# ===========================================================
# Plane of Nightmare - Trash Kill -> Essence Tracking
# ===========================================================
...
# - Keys are zone-specific (PoNightmare_*).
...
sub EVENT_DEATH_COMPLETE {
    ...
    # --- Shared IP grouping (same as CoD version) ---
    ...
    foreach my $pc (@ip_clients) {
        ...
        my $essence_key    = "PoNightmare_EssenceCount_${cid}";
        my $completion_key = "PoNightmare_EssenceComplete_${cid}";

        my $count = quest::get_data($essence_key) || 0;
        $count++;
        quest::set_data($essence_key, $count);

        $pc->Message(15, "You have gathered an essence from the Plane of Nightmare [$count / 1000]");

        if ($count >= 1000) {
            quest::set_data($completion_key, 1);
            $pc->Message(13, "You have gathered 1000 essences from the Plane of Nightmare! A reward chest appears!");
