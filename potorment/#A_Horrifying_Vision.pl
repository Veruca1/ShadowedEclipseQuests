...
sub EVENT_SPAWN {
    ...
    # Treat this as a boss for scaling
    $is_boss = ($raw_name =~ /^#/) ? 1 : 0;
    $npc->SetNPCFactionID(623);
    $npc->ModifyNPCStat("level", 66);

    # Apply default stats + raid scaling
    plugin::DefaultNPCStats($npc, $entity_list);
    plugin::RaidScaling($entity_list, $npc);
    $scaled_spawn = 1;

    # Set HP after scaling
    my $max_hp = $npc->GetMaxHP();
    $npc->SetHP($max_hp) if $max_hp > 0;

    # Start invulnerable until a flagged attacker engages
    $npc->SetInvul(1);
}
...
sub EVENT_DEATH_COMPLETE {
    #quest::signal(207014, 0);   # Tylis_Newleaf
    quest::spawn2(207066, 0, 0, $x, $y, $z, $h); # #Tylis_Newleaf return version
}
