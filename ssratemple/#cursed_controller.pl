...
my $spawntime = 4320 + $variance; # keeping this in case you want it later, but not used for lockouts
...
sub EVENT_TIMER {
    ...
    if ($timer eq "cursed") {
        ...
        if ($check == 0) {
            ...
            quest::stoptimer("one");
            quest::settimer("one", 21600); # 6 hours
        }
    }
...
sub EVENT_SIGNAL {
    ...
    if ($signal == 3) {
        # removed quest::setglobal("cursed_dead",...) so no lockout
        quest::stoptimer("one");
        quest::depop_withtimer();
    }
}