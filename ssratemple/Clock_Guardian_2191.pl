# Clock_Guardian_2191.pl
# Guardian starts invulnerable until within 40 units of 2192, then unlocks
...
sub EVENT_TIMER {
    if ($timer eq "check_guardian" && !$unlocked) {
        my $guardian2 = $entity_list->GetNPCByNPCTypeID(2192);
        if ($guardian2) {
            # 2D distance check (ignore Z)
            my $dx = $npc->GetX() - $guardian2->GetX();
            my $dy = $npc->GetY() - $guardian2->GetY();
            ...
            if ($dist <= $unlock_distance) {
                ...
                $guardian2->ModifyNPCStat("special_abilities", "2,1^3,1^5,1^7,1^8,1^13,1^14,1^15,1^17,1^21,1^31,1^35,0");

                $unlocked = 1; # Prevent repeat unlocking
                # No stoptimer so it continues running in case you want logging/debug
            }
        }
    }
}
