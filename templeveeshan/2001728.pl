# ===========================================================
# #Aaryonar (2001728)
# Temple of Veeshan — Shadowed Eclipse Apex Guardian
# - Applies ToV boss baseline stats
# - Integrates RaidScaling for adaptive raid power
# - Coordinates assistance with nearby dragons
# ===========================================================
...
sub EVENT_COMBAT {
    if ($combat_state == 1) {
        # Aggro Kal`Vunar (124016) and Nir`Tan (124012) if up
        my @npc_ids = (124012, 124016);
        foreach my $npc_id (@npc_ids) {
            my $mob = $entity_list->GetMobByNpcTypeID($npc_id);
            if ($mob && $mob->IsNPC()) {
                $mob->CastToNPC()->AddToHateList($npc, 1);
            }
        }
    }
}
