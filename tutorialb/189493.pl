sub EVENT_SAY {
    if ($text=~/hail/i) {
        ...
        $client->Message(15, "Greetings, traveler, and welcome to Shadowed Eclipse. I am Sir Bard McQuaiden, here to guide you through your adventures. Our server is a place of exploration and discovery, where quests and tasks take a backseat to the thrill of the unknown. While much of Norrath may seem familiar, our custom content is designed to challenge even the most seasoned adventurers. Would you like to hear a tale of our world? [" . quest::saylink("yes", 1) . "] or would you like to [" . quest::saylink("Choose your Starter!", 1) . "]?");
    } 
    elsif ($text=~/^yes$/i) {
    ...
    elsif ($text=~/choose your starter/i) {
        ...
        my $desc = "<b>Charmander:</b> Cauterizing Heal - Heal group for 500 HP, only triggers under 50% HP.<br>
        Charmander's Fire Shield - Applies Damage Shield to All Party Members. Stacks with most Damage Shields.<br><br>
        <b>Bulbasaur:</b> Bulbasaur's Regeneration - Heal 50 HP per Tick.<br>
        Bulbasaur's Thorn Root - Root Effect with DOT. 150 DD, 50 DMG DOT.<br><br>
        <b>Squirtle:</b> Squirtle's Turtle Shell - HP/AC Buff that stacks with other HP/AC buffs.<br>
        Squirtle's Turtle Roar - AE Taunt.<br><br>
        <b>Pikachu:</b> Pikachu's Thunderbolt - 150 DD and 1 second stun every 3 seconds.<br>
        ...
        foreach my $item_id (keys %starter_names) {
            unless (check_player_has_pet($item_id)) {
                my $name = $starter_names{$item_id};
                quest::whisper("Say [" . quest::saylink("$name", 1) . "] if you would like to choose $name as your starter.");
            }
        }
    } 
...
sub EVENT_ITEM {
    if (plugin::check_handin(\%itemcount, 636 => 1)) {
        ...
        $client->Message(15, "You have earned the title 'Newbie No More'!");
