...
sub EVENT_ENTERZONE {
  ...
  if ($ulevel > 85 && $status < 80) {
    $client->Message(25, "You are too high in level to be in this zone.");
    quest::selfcast(2433);
  }  
}
...
sub EVENT_TASK_COMPLETE {
    ...
    # Task 3 completion triggers progress in another task
    if ($task_id == 3) {
        $client->UpdateTaskActivity(34, 2, 1);
        #quest::debug("Task 3 complete! Updating Task 34, Activity 2.");
    }

# Task 35 completion: grant scaled EXP
if ($task_id == 35) {
...
$exp_reward = ceil($exp_reward * $multiplier);  # Always round up
    ...
    # Task 36 completion: remove scavenger relics
    if ($task_id == 36) {
        my @relic_ids = (976, 977, 978, 979, 980);
        foreach my $item_id (@relic_ids) {
