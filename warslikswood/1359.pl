sub EVENT_ITEM {
    ...
    my $flag = "$char_id-remainsofageneral_flag";  # Unique flag identifier for this NPC

    # Check if the player handed in the required item (Item ID 32478)
    if (plugin::check_handin(\%itemcount, 32478 => 1)) {
        # Set the flag indicating the player has completed the hand-in
        quest::set_data($flag, 1);

        plugin::Whisper("The general stirs... You may now hail me to proceed.");
    } else {
...
sub EVENT_SAY {
    my $char_id = $client->CharacterID();  # Get the character's unique ID
    my $flag = "$char_id-remainsofageneral_flag";  # The flag for this NPC
    my $cooldown_key = "$char_id-remainsofageneral_hail_cd";  # A unique cooldown key
    ...
    if ($text =~ /hail/i) {
        if (quest::get_data($flag)) {
            ...
            if (!$last_hail_time || ($current_time - $last_hail_time) > $cooldown_time) {
                ...
                plugin::Whisper("The remains tremble as General Karatukus emerges...");
                # Spawn NPC 1360 (General Karatukus) at the remains' current location
                my $x = $npc->GetX();
                my $y = $npc->GetY();
                my $z = $npc->GetZ();
                my $h = $npc->GetHeading();
                quest::spawn2(1360, 0, 0, $x, $y, $z, $h);
            } else {
